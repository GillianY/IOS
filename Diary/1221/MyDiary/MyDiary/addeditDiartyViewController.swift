...
//    }
    ...
    func addKeyboardToolbar(){
        ...
        var emptyItem1 : UIBarButtonItem =  UIBarButtonItem.init(); //(barButtonSystemItem: nil, target: nil, action: nil);
        var emptyItem2 : UIBarButtonItem =  UIBarButtonItem.init();
        
        //定义完成按钮
       var doneButton : UIBarButtonItem = UIBarButtonItem.init(barButtonSystemItem: UIBarButtonSystemItem.Done, target: self, action: #selector(addeditDiartyViewController.resignKeyboard))
        
        var btnArray = [doneButton ,emptyItem1 , emptyItem2];
