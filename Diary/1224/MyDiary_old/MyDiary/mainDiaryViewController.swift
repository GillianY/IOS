...
class mainDiaryViewController: UIViewController, UITableViewDataSource,UITableViewDelegate {
    ...
    @IBOutlet weak var navigatorSegments: UISegmentedControl!
  ...
  //  @IBOutlet weak var uptoolbar: UIToolbar!
    ...
    @IBAction func segmentCtrlChange(sender: UISegmentedControl) {
       // UISegmentedControl seg = sender;
        ...
        {
            performSegueWithIdentifier("calendarVC", sender: self)
            
        }else if(sender.selectedSegmentIndex == 2)
        {
            performSegueWithIdentifier("addEditDiaryVCS", sender: self)
            
        }else {print("segmentCtrlChange mistake");}
...
//        }
    ...
    override func performSegueWithIdentifier(identifier: String, sender: AnyObject?) {
       ...
       // let segueSender : UISegmentedControl = sender as! UISegmentedControl;  sender : this view Controller
        ...
        let callNewDiary = storyboard?.instantiateViewControllerWithIdentifier("addEditDiaryVCS") as! addeditDiartyViewController ;
            ...
            //sender = callNewDiary;
        }else if(identifier == "calendarVC")
        {
            let calendar = storyboard?.instantiateViewControllerWithIdentifier("calendarVC") as! CaldenarViewController ;
           // calendar
        }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if(segue.identifier == "addEditDiaryVCS")// "addDiary")
        {
            let next = segue.destinationViewController as! addeditDiartyViewController;
            next.actionType = 1; // add 請改成-> stuct
            
        }else if(segue.identifier == "calendarVC")
        {
            let next = segue.destinationViewController as! CaldenarViewController;
